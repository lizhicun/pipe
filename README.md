...
```
补充，当管道的一端被关闭后，下面两条规则将起作用
* 当读一个写端已经被关闭的管道时(child)，所有数据都被读取后，read返回0表示文件结束.(如果管道的写端未关闭，还有进程，就不会产生文件的结束)。
* 如果写一个读端已经被关闭的管道，则产生信号sigpipe。如果忽略该信号或者补货该信号并从处理程序返回，则write返回-1.
dup函数
*
*
### 虚拟文件系统
